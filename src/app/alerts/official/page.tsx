...
import { parseTimestamp, findClosestTimestamp } from '@/utils/timestamp';
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                // Fetch valid timestamps from server to ensure we request a valid one
                // This prevents 404s if the URL timestamp is from the timeline (e.g. 12:05) but data is at 12:00
                ...
                if (timestamps.length > 0) {
                     if (!targetTs || targetTs === 'latest') {
                        // Use latest
                        bestTs = timestamps.sort().pop()!;
                    } else {
                        // Find closest
                         const closest = findClosestTimestamp(
                             targetTs, 
                             timestamps, 
                             120 * 60 * 1000 // 2 hour tolerance
                        );
                        if (closest) {
                            bestTs = closest;
                        } else {
                            // If no close match, maybe just try finding the very latest? 
                            // Or default to latest if the requested one is way off.
                            // For now, let's fall back to latest if the requested specific one is not found
                            // to avoid showing nothing when the user just wants to see alerts.
                            console.warn(`[AlertsPage] Requested timestamp ${targetTs} not found within tolerance. Defaulting to latest.`);
                            bestTs = timestamps.sort().pop()!;
                        }
                    }
                } else {
                ...
                if (bestTs) {
                     // Optimization: Only fetch if changed
                     if (bestTs === lastFetchedTsRef.current && data) {
                         setLoading(false);
                         return;
                     }
                    
                    console.log(`[AlertsPage] Fetching NWS data for targetTs: '${bestTs}' (requested: '${targetTs}')`);
                    
                    setCurrentTimestamp(bestTs);
                    const result = await apiRef.current!.downloadNWS(bestTs);
                    setData(result);
                    lastFetchedTsRef.current = bestTs;
                }
            } catch (err) {
                ...
                if (msg.includes("500")) {
                    ...
                    msg = "Invalid request. The timestamp format may be incorrect or the data is not available.";
                } else if (msg.includes("Invalid timestamp format")) {
                    msg = "Invalid timestamp format. Please use the format: YYYYMMDD-HHMMSS (e.g., 20240128-120000)";
                }
